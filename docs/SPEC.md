...
Only the authentication variables (`CLAUDE_CODE_OAUTH_TOKEN` and `ANTHROPIC_API_KEY`) are extracted from `.env` and passed to the container via stdin. This keeps credentials out of process listings and ensures other environment variables in `.env` are not exposed to the agent.
